...
void LocalStress(Home_t *home, real8 x, real8 y,real8 z,
...
void LocateCell2(Home_t *home, int *cellID, int cellIndex[3], real8 coord[3])
{
        int     cIndex;
 ...
 *      local segments. 
 */
void AllSegmentStress_use_cell(Home_t *home,
 ...
 */
	...
	LocateCell2(home,&cellID,cellIndex,coord);
 ...
 */
  ...
  //      printf("locStress[%d][%d] = %g\n", i, j, locStress[i][j]);
  //      printf("remStress[%d][%d] = %g\n", i, j, remStress[i][j]);
	    }
...
void LocalStress2(Home_t *home, real8 x, real8 y,real8 z,
		 int cellIndex[3],real8 locStress[3][3])
{
...
void AllSegmentStress_no_cell(Home_t *home, real8 xm, real8 ym, real8 zm,
                              real8 totStress[3][3])
{
 ...
 */
        for (i = 0; i < home->newNodeKeyPtr; i++) 
	  {
		...
		int aa, bb, pp, qq, ss, rr;
        	real8   PBCStress[3][3];
		real8 	xAimg,xBimg,yAimg,yBimg;
		real8  	Lx,Ly;
//		Lx = 2*param->xBoundMax;
//		Ly = 2*param->yBoundMax;
		Lx=20000;
		Ly=20000;

//printf("%d %f\n",param->xBoundMax,Ly);

		//Use Only odd numbers(User input)
		NimgPBC = 9;
		//printf("NimgPBC = %d\n",NimgPBC);
		...
		for (aa=0; aa<NimgPBC; aa++) {
		  for (bb=0; bb<NimgPBC; bb++) {
			xAimg = xA+((NimgPBC-1)/2-aa)*Lx;
			xBimg = xB+((NimgPBC-1)/2-aa)*Lx;
			yAimg = yA+((NimgPBC-1)/2-bb)*Ly;
			yBimg = yB+((NimgPBC-1)/2-bb)*Ly;
//printf("xA=%f yA=%f xB=%f yB=%f\n",xAimg,yAimg,xBimg,yBimg);
			SegmentStress(MU, NU, bX, bY, bZ,
				      xBimg, yBimg, zB, xAimg, yAimg, zA,
				      xm, ym, zm, a, sigma);
			for (ss = 0; ss < 3; ss++) 
			  for (rr = 0; rr < 3; rr++)
		...
		real8 stress[6];
		for (i=0; i<6; i++) stress[i] =0.0; 
		StressDueToSeg(xm, ym, zm, xB, yB, zB,
			           xA, yA, zA, bX, bY, bZ,
			           a, MU, NU, stress);
		...
		sigma[2][0] = stress[5];
                ...
                for (mm = 0; mm < 3; mm++) {
                    for (kk = 0; kk < 3; kk++) {
//remove delseg sigmayoffe
#if defined _TF_TEST1 | _TF_TEST2
