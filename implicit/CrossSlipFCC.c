...
void CrossSlipFCC(Home_t *home)
{
		...
		real8  dG, SschmidCS, SschmidGlide, SescaigCS, SescaigGlide;
		real8  glideNormal[3], csNormal[3], linedir[3], csSchmidDir[3], glideSchmidDir[3]; 
		real8  Qcs[3][3], Qglide[3][3], Qcsinv[3][3], Qglideinv[3][3], tmpMat[3][3]; 
		real8  stressCS[3][3], stressGlide[3][3], stress[3][3]; 
		...
		real8	Eb;
		int		doIt;
		real8	Lref = 1.0; //reference length for exponential prefactor
		real8	freq = 2.449e15; //vibrational frequency of dislocation line
#endif
 ...
 */
	   real8  planeLab[3], planeCrystal[3];
	   planeLab[0] = node->nx[0];
	   planeLab[1] = node->ny[0];
	   planeLab[2] = node->nz[0];
	   
	   VECTOR_COPY(planeCrystal, planeLab);
	   
	   if (param->useLabFrame) {
                Matrix33Vector3Multiply(home->rotMatrixInverse, planeLab,
                      				planeCrystal);
	   }

           if ( (fabs(fabs(planeCrystal[0]) - fabs(planeCrystal[1])) > eps) ||
                (fabs(fabs(planeCrystal[1]) - fabs(planeCrystal[2])) > eps) )
		continue;
...
#ifdef _CROSSSLIP
/*
 *						Added by R. B. Sills 2/19/13
 *						If thermally activated cross-slip is enabled, check whether a 
 *						thermally-activated cross-slip event will occur.
 */
						if (param->thermalCrossSlip){

							//Calculate the energy barriers for cross-slip
							EnergyBarrierCrossSlipFCC(home,node,burgCrystal,vec1,newplane,&Eb);

							//Check if thermally-activated cross-slip occurs
							doIt = ThermalActivation(param,Eb,0.5*freq*(L1+L2)/Lref);
