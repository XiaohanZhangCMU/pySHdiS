...
/*
 *      If PARAVIEW file dumps are enabled, do so if the code is
 *      in the termination stage, or it is at the end of a cycle
 *      and either the elapsed simulation time since the last dump
 *      has exceeded the allowable delta time between file dumps
 *      OR the current cycle is a multiple of the specified 
 ...
 */
        if (param->paraview) {
            if (stage == STAGE_TERM) {
                *outputTypes |= GEN_PARAVIEW_DATA;
            } else if (stage == STAGE_CYCLE) {
                if (param->paraviewdt > 0.0) {
                    dumpTime = param->paraviewtime + param->paraviewdt;
                    if (timeNow >= dumpTime) {
                        param->paraviewtime = timeNow;
                        param->paraviewcounter++;
                        *outputTypes |= GEN_PARAVIEW_DATA;
                    }
                } else if ((home->cycle % param->paraviewfreq) == 0) {
                    param->paraviewcounter++;
                    *outputTypes |= GEN_PARAVIEW_DATA;
                }
            }
 ...
 */
        ...
        if ((outputTypes & GEN_PARAVIEW_DATA) != 0) {
            if (stage == STAGE_CYCLE) {
                snprintf(baseName, sizeof(baseName), "paraview%04d",
                         param->paraviewcounter);
            } else {
                snprintf(baseName, sizeof(baseName), "paraview.final");
            }
            WriteParaview(home, baseName, numSegs, binData.segCount);
        }
 ...
 */
        if (((outputTypes & GEN_RESTART_DATA)  != 0) ||
            ...
            ((outputTypes & GEN_PARAVIEW_DATA) != 0)) {
            if (!param->skipIO) {
