 ...
 *
 *      cs.h
 *      This is the header file for the SparseMatrix.c functions. 
 *		These functions are from the CSparse package developed by Tim Davis 
 *		and can be found at:
...
typedef struct cs_sparse    /* matrix in compressed-column or triplet form */
{
    ...
    real8 *x ;     /* numerical values, size nzmax */
    csi nz ;        /* # of entries in triplet matrix, -1 for compressed-col */
} cs ;

cs *cs_add (const cs *A, const cs *B, real8 alpha, real8 beta) ;
csi cs_cholsol (csi order, const cs *A, real8 *b) ;
cs *cs_compress (const cs *T) ;
csi cs_dupl (cs *A) ;
csi cs_entry (cs *T, csi i, csi j, real8 x) ;
csi cs_gaxpy (const cs *A, const real8 *x, real8 *y) ;
cs *cs_load (FILE *f) ;
csi cs_lusol (csi order, const cs *A, real8 *b, real8 tol) ;
cs *cs_multiply (const cs *A, const cs *B) ;
real8 cs_norm (const cs *A) ;
csi cs_print (const cs *A, csi brief) ;
csi cs_qrsol (csi order, const cs *A, real8 *b) ;
...
typedef struct cs_symbolic  /* symbolic Cholesky, LU, or QR analysis */
{
    ...
    real8 lnz ;    /* # entries in L for LU or Cholesky; in V for QR */
    real8 unz ;    /* # entries in U for LU; in R for QR */
} css ;
...
csi cs_droptol (cs *A, real8 tol) ;
csi cs_dropzeros (cs *A) ;
csi cs_happly (const cs *V, csi i, real8 beta, real8 *x) ;
csi cs_ipvec (const csi *p, const real8 *b, real8 *x, csi n) ;
csi cs_lsolve (const cs *L, real8 *x) ;
csi cs_ltsolve (const cs *L, real8 *x) ;
csn *cs_lu (const cs *A, const css *S, real8 tol) ;
cs *cs_permute (const cs *A, const csi *pinv, const csi *q, csi values) ;
csi *cs_pinv (const csi *p, csi n) ;
csi cs_pvec (const csi *p, const real8 *b, real8 *x, csi n) ;
...
csi cs_usolve (const cs *U, real8 *x) ;
csi cs_utsolve (const cs *U, real8 *x) ;
/* utilities */
css *cs_sfree (css *S) ;
csn *cs_nfree (csn *N) ;
csd *cs_dfree (csd *D) ;
...
real8 cs_cumsum (csi *p, csi *c, csi n) ;
...
real8 cs_house (real8 *x, real8 *beta, csi n) ;
...
csi cs_scatter (const cs *A, csi j, real8 beta, csi *w, real8 *x, csi mark,
    cs *C, csi nz) ;
csd *cs_scc (cs *A) ;
csi cs_spsolve (cs *G, const cs *B, csi k, csi *xi, real8 *x,
