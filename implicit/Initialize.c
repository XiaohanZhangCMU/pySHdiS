...
static void Usage(char *program)
{
        printf("\nUsage:  %s [-r <initDLBCycles>] [-s] [-d <data_file>]\\\n"
               "           [-n <numThreads>] [-g <gpuDeviceID>] <ctrl_file>\n\n", program);
 ...
 */
        if (home->myDomain == 0) {
            ...
            if (home->param->paraview) {
                snprintf(subdir, sizeof(subdir), "./%s", DIR_PARAVIEW);
                (void) mkdir(subdir, S_IRWXU);
            }
 ...
 */
            ...
            if (param->forceCutOff && param->fmEnabled) {
                Fatal("Cannot have forceCutOff and fmEnabled toggled"
                        " on simultaneously!\n");
            }
 ...
 */
        if (strcmp(param->mobilityLaw, "BCC_0") == 0) {
            ...
            param->numBurgGroups = 7;
#ifdef _IMPLICIT
		} else if (strcmp(param->mobilityLaw, "Glide") == 0) {
            param->materialType = MAT_TYPE_GLIDE;
            param->mobilityType = MOB_GLIDE;
            param->mobilityFunc = Mobility_Glide;
            param->numBurgGroups = 7;
#endif
        } else {
...
#ifdef _GPU_SUBCYCLE
        if (home->deviceID == -1) {
			printf("**** GPU device ID:   to be selected\n");
		} else {
 ...
 */
        ...
        if (home->myDomain != 0) {
            subcyc = home->subcyc = (Subcyc_t *)calloc(1, sizeof(Subcyc_t));
...
#ifndef _GPU_SUBCYCLE
                    printf("WARNING: Not compiled with GPU support; "
                           "'-g' option ignored.\n");
#endif
...
#ifndef FULL_N2_FORCES
/*
 *		This part initializes the long-range image tables if cut-off radius
 *		is not implemented.
 */
		if (param->forceCutOff == 0) {
