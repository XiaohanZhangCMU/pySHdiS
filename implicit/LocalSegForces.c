                             ...
                             int *segPairListCnt, int *segPairListSize)
{
        if (*segPairListCnt == *segPairListSize) {
            //In parallel, if this increment size is too small the 
            //performance can degrade significantly!
            *segPairListSize = MAX(2*(*segPairListSize), 10000);
                   ...
                   real8 *f3, real8 *f4)
{
        ...
        real8   z1, z2, z3, z4;
        real8   dx, dy, dz;
        real8   cutoff1s, b12x, b12y, b12z;
        real8   cutoff2s, b34x, b34y, b34z;
		real8   dist2, L1, fraction, pi_cutoff21, dist , L2, ddist2dt; 
		real8   PI = 3.14159265358979;
        Param_t *param;
        Cell_t  *cell;
#ifdef _FEM
        int     i;
 ...
 */
		...
		// Check if the distance between the segments is less than the cutoff radius
		if (param->forceCutOff) {
			cutoff1s = param->cutoff1 * param->cutoff1;
			cutoff2s = param->cutoff2 * param->cutoff2;
			
			if ((node1->myTag.domainID == node3->myTag.domainID  &&
			     node1->myTag.index    == node3->myTag.index   ) ||
			    (node2->myTag.domainID == node3->myTag.domainID  &&
			     node2->myTag.index    == node3->myTag.index   ) ||
			    (node2->myTag.domainID == node4->myTag.domainID  &&
			     node2->myTag.index    == node4->myTag.index   ) ||
			    (node1->myTag.domainID == node4->myTag.domainID  &&
			     node1->myTag.index    == node4->myTag.index   )) {
				dist2 = 0.0;
		//	} else if ((fabs(x1+x2-x3-x4)/2.0 > param->cutoff2) ||
		//	           (fabs(y1+y2-y3-y4)/2.0 > param->cutoff2) ||
		//	           (fabs(z1+z2-z3-z4)/2.0 > param->cutoff2) ) {
		//		dist2 = cutoff2s + 1.0;
			} else {
			...
			if ( dist2 >= cutoff2s) {
				// If the distance is more than outer cutoff, ignore the interaction.
				VECTOR_ZERO(f1);
				VECTOR_ZERO(f2);
				VECTOR_ZERO(f3);
				VECTOR_ZERO(f4);
			...
			} else if (dist2 > cutoff1s) {
				pi_cutoff21 = PI/( param->cutoff2 - param->cutoff1 );
				
				// Calculating the force fraction used for applying smooth cutoff
				dist = sqrt(dist2);
				fraction = 0.5 * (1.0 + cos(pi_cutoff21 * (dist - param->cutoff1)));
			}
		}
 ...
 */
        if (node1->constraint == SURFACE_NODE ||
            node1->constraint == THINFILM_SURFACE_NODE || 
 ...
 */
        if (node2->constraint == SURFACE_NODE ||
            node2->constraint == THINFILM_SURFACE_NODE || 
 ...
 */
        if (node3->constraint == SURFACE_NODE ||
            node3->constraint == THINFILM_SURFACE_NODE || 
 ...
 */
        if (node4->constraint == SURFACE_NODE ||
            node4->constraint == THINFILM_SURFACE_NODE || 
            node4->constraint == CYLINDER_SURFACE_NODE) {
        ...
        if ( (param->forceCutOff) && (dist2 > cutoff1s) ) {
			f1[0] *= fraction ;  f1[1] *= fraction ;  f1[2] *= fraction ;
			f2[0] *= fraction ;  f2[1] *= fraction ;  f2[2] *= fraction ;
			f3[0] *= fraction ;  f3[1] *= fraction ;  f3[2] *= fraction ;
			f4[0] *= fraction ;  f4[1] *= fraction ;  f4[2] *= fraction ;
		}
...
#ifdef _SHDIS
/*
 *                  No remote forces are calculated except if _SHDIS
 *                  is enabled. Then  we need to add in force from
 *                  the remote stress coming from the SH contribution only.
 */
                    sigb[0] = node1->sigbRem[armID12*3];
                    sigb[1] = node1->sigbRem[armID12*3+1];
                    sigb[2] = node1->sigbRem[armID12*3+2];
...
/*                  When _SHDIS is enabled, we need to add in
 *                  remote stress from the SH contribution
...
#ifdef _GPU_SUBCYCLE
/*
 *      If we are using option _GPU_SUBCYCLE, compute the pair 
 *      interactions forces on the GPU to accelerate the calculation.
 *      The forces are stored within array fpair and will be retreived
 *      from there during the segment pairs loop below.
 */
		double *fpair = (double*)malloc(sizeof(double)*12*segPairListCnt);
		LocalSegForcesGPU(home, segPairListCnt, segPairList, fpair);
#endif
