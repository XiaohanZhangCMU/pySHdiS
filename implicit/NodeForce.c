...
void ZeroNodeForces(Home_t *home, int reqType)
{
	...
	/*	#pragma omp parallel for default(none) schedule(dynamic,1) \*/
		...
		for (i = 0; i < home->newNodeKeyPtr; i++) {
			...
			if (node->subgroup == 0 && reqType>=GROUP0) continue;
/*
 *          If we're doing a FULL zeroing of forces, or the node is flagged
 *          to have forces reset, zero it's toal forces.
 */
            if ((reqType != PARTIAL) || (node->flags & NODE_RESET_FORCES) != 0) {
                node->fX = 0.0;
                node->fY = 0.0;
                node->fZ = 0.0;
            }
 ...
 */
		...
		while (node) {
			if (node->subgroup == 0 && reqType>=GROUP0) {
				node = node->next;
				continue;
			}
...
{
        int      i, nc, ti, elasticity, nbrArm, armID12, armID21;
        real8    f1[3], f2[3];
        Node_t   *node1, *node2, *node, *nbr;
        Param_t  *param;
		Subcyc_t *subcyc;
		Segm_t   *SegList;
		int       SegListSize;
...
/*
 *		Reset node forces as necessary based on the reqType flag
 */
 ...
 */
        if (!elasticity) {
			...
			if (reqType >= GROUP0) {
				...
				if (reqType == GROUP0) {
					SegListSize = subcyc->SegListG0_cnt;
					SegList     = subcyc->SegListG0    ;
				} else if (reqType == GROUP1) {
					SegListSize = subcyc->SegListG1_cnt;
				...
				for (i = 0; i < SegListSize ; i++) {
					...
					LineTensionForce(home, node1->x, node1->y, node1->z,
										   ...
										   node1->burgX[armID12], node1->burgY[armID12], 
										   node1->burgZ[armID12], f1, f2);

					AddtoNodeForce(node1, f1);
					AddtoArmForce (node1, armID12, f1);

					if (node2->myTag.domainID == home->myDomain) {
						AddtoNodeForce(node2, f2);
...
/*    
 *    During sucycling, the far-field sigbRem contribution  
 *    will be computed .
 */
	if (reqType == PARTIAL || reqType == FULL || reqType == GROUP1)
	{
#ifdef _SHDIS
/*
 ...
 *
 *      When _SHDIS is enabled, then we also need 
 *      to add the SH image stress to each segment's sigbRem
 *      even to full N2 forces calculations, and even when FMM is on.
 *
 *      Exception:
 *        - If full n^2 seg/seg forces are being calculated, we don't
 *          do any remote force calcs... unless the SH code is enabled,
...
#ifdef _SHDIS
/* 
 *    When _SHDIS is enabled, we ALWAYS conpute the SH contribution
 *    of sigbRem. sigbrem is used later in LocalSegForces.
 */
		ComputeSHSegSigbRem(home, FULL);
#endif
	}
 ...
 *
 *		All force calculations for a given segment will be calculated
 ...
 */
		...
		if (reqType == GROUP0) {
			NodeForceList2(home, subcyc->SegSegListG0, subcyc->SegSegListG0_cnt, 
			                     subcyc->SegListG0   , subcyc->SegListG0_cnt   , reqType);
		
		} else if (reqType == GROUP1) {
			NodeForceList2(home, subcyc->SegSegListG1, subcyc->SegSegListG1_cnt,
			                     subcyc-> SegListG1  , subcyc->SegListG1_cnt, reqType);

		} else if (reqType == GROUP2) {
			NodeForceList2(home, subcyc->SegSegListG2, subcyc->SegSegListG2_cnt, 
			                     NULL, 0, reqType);

		} else if (reqType == GROUP3) {
			NodeForceList2(home, subcyc->SegSegListG3, subcyc->SegSegListG3_cnt, 
			                     NULL, 0, reqType);

		} else if (reqType == GROUP4) {
			NodeForceList2(home, subcyc->SegSegListG4, subcyc->SegSegListG4_cnt, 
			                     NULL, 0, reqType);
			
		} else if (param->forceCutOff && (strcmp(param->timestepIntegrator, "nodeBsubcycle") == 0)) {
			ZeroLongRangeNodeForces(home, FULL);
			NodeForceList (home, subcyc->ShortRange_SegSegList , subcyc->Size_SegSegList , reqType);
...
{
		int     i, j, k, l, cx, cy, cz, cell2Index, cell2X, cell2Y, cell2Z;
		int     armID12, armID21, armID34, armIndex , nbrCell2Index, nextIndex;
		real8   a, MU, NU, Ecore, eps, dCutOf;
		real8   dx, dy, dz;
...
/*
 *          When the SH is enabled and we are doing N2 forces calcs, 
 *          we need to explicitly zero sigbRem and add in remote stress from
 *          SH in later in this function.
 */
 /*
...
#ifdef _SHDIS
	    /* When _SHDIS is on, we add the remote stress contribution
	     *  in all cases: FMM on, Full N2 on etc.
             * Here we are in FULL_N2
	     */

            ComputeSH1SegSigbRem(home, node1, node2, armID12, armID21);
 ...
 *									And make sure we don't try to calculate seg/seg forces
...
/*
 *									And after computing forces between the two segments,
 ...
 */
							...
							}  /* while (nextindex) (node3) */
						}  /* for (cz) */
					}  /* for (cy) */
				}  /* for (cx) */
			} else {

				//  If cutoff in not implemented loop through all the 
				//  segments and do any necessary seg/seg force calcs.
 ...
 */
/*
 *          If we need to add in any SH image stress, 
 *          or we're not using the fmm code,  we need to explicitly 
 *          zero out the segment sigbRem. SH contribution is added later
 *          in this function.
 ...
 *          component from remote stress. FMM arm forces must not be 
 *          added here because they are updated within RemoteForceOneSeg().
 */
            VECTOR_ZERO(f1);
            VECTOR_ZERO(f2);

            if (param->fmEnabled && param->forceCutOff == 0) {
...
#if !defined _FEM & !defined _SHDIS
/*
 *              If we are not linked with FEM and SH, calculate the PK force
...
#ifdef _SHDIS
/*
 *          When linked with SH, we have to add the SH image stress
...
/*-------------------------------------------------------------------------
 *
 *      Function:       SetTwoNodesForce
 *      Description:    This function is similar to SetOneNodeForce but
 *                      optimized for GPU calculation. Specifically, the 
 *                      force is recalculated for all segments attached
 *                      to a pair of specified nodes to minimize memory
 *                      transfer from host to device. After all segment 
 *                      forces are computed, total forces are resummed 
 *                      for the nodes attached to the segments.
 *
 *------------------------------------------------------------------------*/
void SetTwoNodesForce(Home_t *home, Node_t *nodeOne, Node_t *nodeTwo)
{
        int     armIndex, nodeIndex, i;
        int     segPairCnt, segPairListSize;
        real8   a, MU, NU, Ecore, eps;
        real8   sigb[3], totRemSig[3][3], extstress[3][3];
        real8   fseg1node1[3], fseg1node2[3];
        ...
        int segPairNode[2];
        real8 ***armForceBck = (real8***)malloc(2*sizeof(real8**));
        
/*
 * 		Assign nodes indices that will be used to locate
 *      the nodes in the GPU node vector
 */
        int nodeCount = 0;
		for (i = 0; i < home->newNodeKeyPtr; i++) {
			if ((nodeA = home->nodeKeys[i]) == (Node_t *)NULL) continue;
			nodeA->subindex = nodeCount;
			nodeCount++;
		}
        
        SplitSegSeg_t *splitSegSegList;
        splitSegSegList = (SplitSegSeg_t*)malloc(sizeof(SplitSegSeg_t));
        
        splitSegSegList->nodeCount = nodeCount;
        
        int nArms = nodeOne->numNbrs + nodeTwo->numNbrs;
        splitSegSegList->n12 = (int*)malloc(2*nArms*sizeof(int));
        splitSegSegList->cc = (double*)malloc(3*nArms*sizeof(double));
        splitSegSegList->b1 = (double*)malloc(3*nArms*sizeof(double));
        
        splitSegSegList->nArms = 0;
        splitSegSegList->n34 = (int*)NULL;
        splitSegSegList->b2 = (double*)NULL;
        
/*
 *      Loop over the two nodes for which we want to recalculate forces
 */        
        for (nodeIndex = 0; nodeIndex < 2; nodeIndex++) {
			...
			armForceBck[nodeIndex] = (real8**)malloc(nodeA->numNbrs*sizeof(real8*));
 ...
 */
			for (armIndex = 0; armIndex < nodeA->numNbrs; armIndex++) {
				...
				armForceBck[nodeIndex][armIndex] = (real8*)calloc(6,sizeof(real8));
 ...
 */
				...
				if (param->fmEnabled && param->forceCutOff == 0) {
					...
					/* Here we need to add the contribution to the force
					 * vectors as the arm forces will be reset later */
					VECTOR_ADD(fseg1node1, f1);
					VECTOR_ADD(fseg1node2, f2);
				}
...
/*
 *              Save the forces accumulated so far into the segment's
 *              back up forces for later use
 */
				armForceBck[nodeIndex][armIndex][0] = fseg1node1[0];
				armForceBck[nodeIndex][armIndex][1] = fseg1node1[1];
				armForceBck[nodeIndex][armIndex][2] = fseg1node1[2];
				armForceBck[nodeIndex][armIndex][3] = fseg1node2[0];
				armForceBck[nodeIndex][armIndex][4] = fseg1node2[1];
				armForceBck[nodeIndex][armIndex][5] = fseg1node2[2];
 ...
 */
									if (segPairCnt == segPairListSize) {
										segPairListSize += 10000;
										segPairList = (SegPair_t *)realloc(
												segPairList, sizeof(SegPair_t) *
												segPairListSize);
										
										splitSegSegList->n34 = (int*)realloc(splitSegSegList->n34, 
															   sizeof(int)*segPairListSize*3);
										splitSegSegList->b2 = (double*)realloc(splitSegSegList->b2, 
															  sizeof(double)*segPairListSize*3);
									}

									segPairList[segPairCnt].node1 = node1;
									segPairList[segPairCnt].node2 = node2;
									
									segPairList[segPairCnt].armID12 = armID12;
									segPairList[segPairCnt].armID21 = armID21;
									
									splitSegSegList->n34[segPairCnt*3+0] = splitSegSegList->nArms;
									splitSegSegList->n34[segPairCnt*3+1] = node3->subindex;
									splitSegSegList->n34[segPairCnt*3+2] = node4->subindex;
									splitSegSegList->b2[segPairCnt*3+0] = node3->burgX[armID34];
									splitSegSegList->b2[segPairCnt*3+1] = node3->burgY[armID34];
									splitSegSegList->b2[segPairCnt*3+2] = node3->burgZ[armID34];
				...
				splitSegSegList->cc[splitSegSegList->nArms*3+0] = xCenter;
				splitSegSegList->cc[splitSegSegList->nArms*3+1] = yCenter;
				splitSegSegList->cc[splitSegSegList->nArms*3+2] = zCenter;
				splitSegSegList->b1[splitSegSegList->nArms*3+0] = node1->burgX[armID12];
				splitSegSegList->b1[splitSegSegList->nArms*3+1] = node1->burgY[armID12];
				splitSegSegList->b1[splitSegSegList->nArms*3+2] = node1->burgZ[armID12];
				splitSegSegList->n12[splitSegSegList->nArms*2+0] = node1->subindex;
				splitSegSegList->n12[splitSegSegList->nArms*2+1] = node2->subindex;
				
				splitSegSegList->nArms++;
				
			}  /* for (armIndex = 0; ...) */
						   
			segPairNode[nodeIndex] = segPairCnt;
        
		} /* for (nodeIndex = 0; ...) */

/*
 *      Calculate the forces between every segment pair in the list
 *      created above, and update the forces on segment 1. The forces
 *      calculation are performed on the GPU device.
 */
		splitSegSegList->segPairCnt = segPairCnt;
		splitSegSegList->fpair = (double*)malloc(sizeof(double)*6*segPairCnt);
		
		SetOneNodeForceGPU(home, splitSegSegList);

/*
 *      Assemble all forces on nodes
 */        
        for (nodeIndex = 0; nodeIndex < 2; nodeIndex++) {
			int threadIterStart, threadIterEnd;
			
			if (nodeIndex == 0) {
				nodeA = nodeOne;
				threadIterStart = 0;
				threadIterEnd = segPairNode[0];
			} else {
				nodeA = nodeTwo;
				threadIterStart = segPairNode[0];
				threadIterEnd = segPairNode[1];
			}
...
/*
 *              We should be able to calculate new forces for the segment, so
 *              zero out the old segment forces and add the forces saved in
 *              the segments back up force array.
 */
				node1->armfx[armID12] = 0.0;
				node1->armfy[armID12] = 0.0;
				node1->armfz[armID12] = 0.0;
				...
				fseg1node1[0] = armForceBck[nodeIndex][armIndex][0];
				fseg1node1[1] = armForceBck[nodeIndex][armIndex][1];
				fseg1node1[2] = armForceBck[nodeIndex][armIndex][2];
				fseg1node2[0] = armForceBck[nodeIndex][armIndex][3];
				fseg1node2[1] = armForceBck[nodeIndex][armIndex][4];
				fseg1node2[2] = armForceBck[nodeIndex][armIndex][5];
            ...
            for (pairID = threadIterStart; pairID < threadIterEnd; pairID++) {
                ...
                armID12 = segPairList[pairID].armID12;
				armID21 = segPairList[pairID].armID21;
				
/*
 *              Update forces on segment 1
 */
                fseg1node1[0] = splitSegSegList->fpair[pairID*6+0];
                fseg1node1[1] = splitSegSegList->fpair[pairID*6+1];
                fseg1node1[2] = splitSegSegList->fpair[pairID*6+2];

                fseg1node2[0] = splitSegSegList->fpair[pairID*6+3];
                fseg1node2[1] = splitSegSegList->fpair[pairID*6+4];
                fseg1node2[2] = splitSegSegList->fpair[pairID*6+5];
 ...
 */
			...
			for (armIndex = 0; armIndex < nodeA->numNbrs; armIndex++) {
				...
				free(armForceBck[nodeIndex][armIndex]);
        ...
        if (segPairList != (SegPair_t *)NULL) {
            free(segPairList);
            free(splitSegSegList->n34);
            free(splitSegSegList->b2);
        }
        
		free(splitSegSegList->n12);
		free(splitSegSegList->cc);
		free(splitSegSegList->b1);
		
		free(splitSegSegList->fpair);
		free(splitSegSegList);
