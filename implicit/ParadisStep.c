...
#ifdef _DEBUG_GLIDE_VIOLATIONS
        TestForGlideViolations(home,"beginning of step");
#endif   

        param = home->param;
        
/*
 *		Generating the seg-seg list for short-range force calculations
 */
		if ((strcmp(param->timestepIntegrator, "nodeBsubcycle") == 0) &&
		    (param->forceCutOff)) {
			SegSegList_For_Force (home);
		}
 ...
 */
		...
		////////////////////////////////////////////////
	/*	FILE   *fp;
		int     sGroup, mobIterError;
		struct  timeval  WCT0 , WCT1 , WCT2;
		real8   dWCT1, dWCT2;
		int     homeDomain = home->myDomain;
		int     numDomains = home->numDomains;
		char    str[80];
		
		if (homeDomain == 0) {
			gettimeofday( &WCT0, NULL );
			printf("SegSegListMaker started\n");
		}
		SegSegListMaker(home, FULL);
		MPI_Barrier(MPI_COMM_WORLD);
			
		if (homeDomain == 0) {
			gettimeofday( &WCT1, NULL );
			dWCT1 = (WCT1.tv_sec - WCT0.tv_sec) + (WCT1.tv_usec - WCT0.tv_usec)/1000000.0;
			printf("dWCT1 = %f \n", dWCT1);
		}
		
		for (i=0 ; i<5 ; i++) {
			if      (i==0) sGroup = GROUP0;
			else if (i==1) sGroup = GROUP1;
			else if (i==2) sGroup = GROUP2;
			else if (i==3) sGroup = GROUP3;
			else if (i==4) sGroup = GROUP4;
			
			if (homeDomain == 0) {
				printf("--------------------------------\n");
				printf("Group %i started\n", i);
				gettimeofday( &WCT1, NULL );
			}
			
			FlagSubcycleNodes(home, sGroup);			
			NodeForce(home, sGroup);
			MPI_Barrier(MPI_COMM_WORLD);
			if (homeDomain == 0) {
				gettimeofday( &WCT2, NULL );
				dWCT2 = (WCT2.tv_sec - WCT1.tv_sec) + (WCT2.tv_usec - WCT1.tv_usec)/1000000.0;
				printf("dWCT2 = %f \n", dWCT2);
			}
			
			mobIterError = CalcNodeVelocities(home, 0, 1);
			CommSendVelocitySub(home, sGroup);
			
			if (sGroup==GROUP0) {
				printf("homeDomain=%d , numDomains=%d , SegSegListG0_cnt= %d , SegListG0_cnt= %d \n", 
						homeDomain , numDomains , home->subcyc->SegSegListG0_cnt , home->subcyc->SegListG0_cnt);
			} else if (sGroup==GROUP1) {
				printf("homeDomain=%d , numDomains=%d , SegSegListG1_cnt= %d , SegListG1_cnt= %d \n", 
						homeDomain , numDomains , home->subcyc->SegSegListG1_cnt , home->subcyc->SegListG1_cnt);
			} else if (sGroup==GROUP2) {
				printf("homeDomain=%d , numDomains=%d , SegSegListG2_cnt= %d \n", 
						homeDomain , numDomains , home->subcyc->SegSegListG2_cnt);
			} else if (sGroup==GROUP3) {
				printf("homeDomain=%d , numDomains=%d , SegSegListG3_cnt= %d \n", 
						homeDomain , numDomains , home->subcyc->SegSegListG3_cnt);
			} else if (sGroup==GROUP4) {
				printf("homeDomain=%d , numDomains=%d , SegSegListG4_cnt= %d \n", 
						homeDomain , numDomains , home->subcyc->SegSegListG4_cnt);
			}
			
			if (numDomains == 1) sprintf(str, "G%i/bbb"   , i );
			else                 sprintf(str, "G%i/bbb%i" , i , homeDomain);
			fp = fopen(str, "w");
			
			int j;
			for (j = 0; j < home->newNodeKeyPtr; j++) {
				if ((node = home->nodeKeys[j]) == (Node_t *)NULL) continue;
...
/*
 *      Pre-compute glide constraints into mobility matrix
 *      Only available for FCC_0 mobility law
 */		
		if (param->mobilityType == MOB_FCC_0) {
			param->preMobilityConstraints = 1;
			
			for (i = 0; i < home->newNodeKeyPtr; i++) {
				if ((node = home->nodeKeys[i]) == (Node_t *)NULL) continue;
				int mobErr = Mobility_FCC_0_pre_matrix(home, node);
			}
			...
			param->preMobilityConstraints = 0;
		}
#endif
 ...
 */
		for (i = 0; i < home->newNodeKeyPtr; i++) {
			...
			node->oldery = node->y;
			node->olderz = node->z;
			node->oldervX = node->vX;
			node->oldervY = node->vY;
			node->oldervZ = node->vZ;
		}
		node = home->ghostNodeQ;
		while (node) {
			node->olderx = node->x;
 ...
 */
		TimerStart(home, TIME_INTEGRATE);
		...
		} else if (strcmp(param->timestepIntegrator, "RKF"           ) == 0) {
            RKFIntegrator(home, FULL);
        } else if (strcmp(param->timestepIntegrator, "implicit"      ) == 0) {
            ImplicitIntegrator(home);
        } else if (strcmp(param->timestepIntegrator, "forceBsubcycle") == 0) {				
			SubcycleIntegratorForceB(home);
		} else if (strcmp(param->timestepIntegrator, "nodeBsubcycle" ) == 0) {
			SubcycleIntegratorNodeB(home);
        } else {
/*
 ...
 */
		...
		TimerStop(home, TIME_INTEGRATE);
		

#ifdef _PRECOMPUTE_GLIDE		
/*
 *      Do not use pre-computed glide constraints for the rest
 * 		of the cycle (node splitting, collisions, remesh, etc...)
 */		
		param->preMobilityConstraints = 0;
#endif

/*
 *		Try to repair any glide plane constraints that have been violated
 *      during time integration. (This function requires valid old positions.)
 */
		FixGlideViolations(home,NULL,NULL);

#ifdef _DEBUG_GLIDE_VIOLATIONS
        TestForGlideViolations(home,"time integration");
#endif
...
/*
 *      Sort nodes into cells for collision detection.
 */
...
#ifdef _DEBUG_GLIDE_VIOLATIONS
        TestForGlideViolations(home,"split multinodes");
#endif
...
#ifdef _DEBUG_GLIDE_VIOLATIONS
        TestForGlideViolations(home,"cross-slip");
#endif
...
#ifdef _DEBUG_GLIDE_VIOLATIONS
        TestForGlideViolations(home,"collisions");
#endif
...
#ifdef _DEBUG_GLIDE_VIOLATIONS
        TestForGlideViolations(home,"remesh");
#endif
...
#ifdef _DEBUG_GLIDE_VIOLATIONS
        TestForGlideViolations(home,"end of step");
#endif
