...
/*
 *      Parameters related to cut-off distances for N^2 force calculations
 */
        BindVar(CPList, "cutoff1"    , &param->cutoff1    , V_DBL, 1, VFLAG_NULL);
		param->cutoff1 = 1e10;
		
		BindVar(CPList, "cutoff2"    , &param->cutoff2    , V_DBL, 1, VFLAG_NULL);
		param->cutoff2 = 1e10;
		
		BindVar(CPList, "forceCutOff", &param->forceCutOff, V_INT, 1, VFLAG_NULL);
		param->forceCutOff = 0;
