 ...
 *                   separated nodes in any proximity-based calculation.
 *					 The primary use is for collision handling, but
 *					 this is also used by the implicit integrator 
 *					 and subcycling integrator. This function is 
 *					 based off of the SortNodesForCollision function.
 *
...
void SortNodes(Home_t *home, real8 *maxsep)
{
        ...
        int     ix, iy, iz, i, j, baseIdx, cell2Idx, cellIdx;
		...
		real8	localmaxsep[1], globalmaxsep[1];
 ...
 */   
		if (*maxsep==0.0) {
			if (param->collisionMethod==3) {
				real8	dx, dy, dz, dr2, dr2max, l2, l2max;
				...
				localmaxsep[0] = sqrt(4*dr2max+l2max/2) + param->rann;
#ifdef PARALLEL
				MPI_Allreduce(localmaxsep, globalmaxsep, 1, MPI_DOUBLE, MPI_MAX, 
                              MPI_COMM_WORLD);
				*maxsep = globalmaxsep[0];
#else	
				*maxsep = localmaxsep[0];
#endif
				//*maxsep = sqrt(4*dr2max+pow(param->maxSeg/sqrt(2),2)) + param->rann;
				//*maxsep = 2*sqrt(dr2max) + 1.5*0.5*param->maxSeg + param->rann;
			} else {
		    	*maxsep = (1.1 * param->maxSeg) + param->rann;
			}
            home->cell2size = *maxsep;
		}
//*maxsep = (1.1 * param->maxSeg) + param->rann;
