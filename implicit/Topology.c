...
/*
 *						Modified by R. B. Sills, 1/28/14
 *						For a glide plane to be assigned, we need to have that
 *						the segment has the same Burgers vector as a neighbor
 *						AND that the segment resides in that plane. Otherwise, a
 *						two-armed node with arms on different planes may pick
 *						the wrong glide plane of the two.
 */
						real8	dr[3], n[3];
						int		planetest;
						Node_t	*tmpNbr;
						...
						param = home->param;
                     	...
                     	dr[0] = tmpNbr->x-node->x;
                      	dr[1] = tmpNbr->y-node->y;
                      	dr[2] = tmpNbr->z-node->z;
					  	ZImage(param, &dr[0], &dr[1], &dr[2]);
					  	NormalizeVec(dr);	
                          ...
                          for (arm = 0; arm < node->numNbrs; arm++) {
                              ...
                              if (DotProduct(bsum, bsum) < 1.0e-4) {
								  if (param->enforceGlidePlanes) {
									  //Test if the segment resides on this glide
	   								  //plane.
									  n[0] = node->nx[arm];
									  n[1] = node->ny[arm];
									  n[2] = node->nz[arm];
									  NormalizeVec(n);
									  planetest = (fabs(DotProduct(dr,n)) < 1.0e-4);
								  } else {
								      planetest = 1;
								  }
								  if (planetest) {
                        ...
                        ChangeArmBurg(home, node, &nbrNode->myTag,
                                      ...
                                      globalOp, DEL_SEG_HALF);
/*
 *			Added by R. B. Sills, 7/8/14
 *			ChangeArmBurg may have selected a glide plane for the arm.
 *			In order to prevent the other arm from having a different
 *			glide plane, set nx, ny, and nz to the values of the node's
 *			arm.
 */
			int k;
 ...
 */
                GetMinDist2(x1, y1, z1, vx1, vy1, vz1,
...
#ifdef _GPU_SUBCYCLE
/*
 *      When option _GPU_SUBCYCLE is enabled, the calculation of
 *      nodal forces involved in the SplitMultiNodes procedure are
 *      accelerated using the GPU. First, initialize some parameters 
 *      and allocate memory on the GPU device.
 */
		InitializeNodeForceGPU(home);
#endif
...
/*
 *						For the first velocity calculation, we zero out the 
 *						glide plane of the zero-length segment connecting the 
 *						new nodes - this prevents any glide constraints from
 *						being enforced in the mobility law.
 */
						real8 burg[3], dirvec[3], nplane[3];
						int armIndex1, armIndex2;

						armIndex1 = GetArmID(home, splitNode1, splitNode2);
						armIndex2 = GetArmID(home, splitNode2, splitNode1);
						
						if (armIndex1 != -1) {
		                    splitNode1->nx[armIndex1] = 0.0;
		                    splitNode1->ny[armIndex1] = 0.0;
		                    splitNode1->nz[armIndex1] = 0.0;

		                    splitNode2->nx[armIndex2] = 0.0;
		                    splitNode2->ny[armIndex2] = 0.0;
		                    splitNode2->nz[armIndex2] = 0.0;
						}
...
/*
 *						Now we have a splitting direction, given by the nodal
 *						velocities, and we can define a glide plane for the new
 *						segment. This glide plane may be different from the
 *						plane defined by b x v (see the FindPreciseGlidePlane
 *						function for details), so after setting the glide plane,
 *						recalculate the splitting velocities.
 */
                        if (mobError == 0) {
 ...
 */
						  //real8 epsvel = 1e-3; //eps is too small for this test
							//should epsvel scale with the mobility?
						  //Based on the velocity with an applied stress of 0.001 MPa
						  real8 epsvel = 0.001e6*param->MobEdge;
	
						  if ((sqrt(vd1) > epsvel) || (sqrt(vd2) > epsvel)) {
                            if (vd1 > vd2) {
							...
							if (armIndex1 != -1) { //make sure this arm exists
				                burg[X] = splitNode1->burgX[armIndex1];
				                burg[Y] = splitNode1->burgY[armIndex1];
				                burg[Z] = splitNode1->burgZ[armIndex1];
		                        ...
		                        splitNode1->nx[armIndex1] = nplane[X];
		                        splitNode1->ny[armIndex1] = nplane[Y];
		                        splitNode1->nz[armIndex1] = nplane[Z];

		                        splitNode2->nx[armIndex2] = nplane[X];
		                        splitNode2->ny[armIndex2] = nplane[Y];
		                        splitNode2->nz[armIndex2] = nplane[Z];
...
/*
 *                      	Now we can assess the split because all glide
 ...
 */
							  ...
							  if ((sqrt(vd1) > epsvel) || (sqrt(vd2) > epsvel)) {
		                        if (vd1 > vd2) {
                                    oldpos[X] = splitNode1->x;
                                    oldpos[Y] = splitNode1->y;
                                    oldpos[Z] = splitNode1->z;
		                            invvNorm = 1.0 / sqrt(vd1);
		                            dirx = -splitNode1->vX * invvNorm;
		                            diry = -splitNode1->vY * invvNorm;
		                            dirz = -splitNode1->vZ * invvNorm;
		                            ...
		                            FixGlideViolations(home,&splitNode1->myTag,oldpos);
		                            FoldBox(param, &splitNode1->x, &splitNode1->y,
		                                    &splitNode1->z);
		                        } else {
                                    oldpos[X] = splitNode2->x;
                                    oldpos[Y] = splitNode2->y;
                                    oldpos[Z] = splitNode2->z;
		                            invvNorm = 1.0 / sqrt(vd2);
		                            dirx = splitNode2->vX * invvNorm;
		                            diry = splitNode2->vY * invvNorm;
		                            dirz = splitNode2->vZ * invvNorm;
		                            ...
		                            FixGlideViolations(home,&splitNode2->myTag,oldpos);
...
#if _GPU_SUBCYCLE
/*
 *                              Recalculate nodal forces on the GPU device simultaneously 
 *                              for both nodes involved in the current split set
 */
		                        SetTwoNodesForce(home, splitNode1, splitNode2);
#else
...
//									Added by R. B. Sills, 1/9/14
                                    ...
                                    armIndex1Max = armIndex1;
                                    armIndex2Max = armIndex2;

                                    if (armIndex1 != -1) {
                                        nplaneMax[X] = nplane[X];
                                        nplaneMax[Y] = nplane[Y];	
                                        nplaneMax[Z] = nplane[Z];
                                    }		
////////////////////////////////////////////////////////////////////////////////								
 ...
 */
////////////////////////////////////////////////////////////////////////////////
//			Removed by R. B. Sills, 1/9/14
//			Nodal positions already stored above in pos1 and pos2.
/*
                        vd1 = vel1[X] * vel1[X] +
                              vel1[Y] * vel1[Y] +
                              vel1[Z] * vel1[Z];
...
*/
///////////////////////////////////////////////////////////////////////////////
//printf("split node %i into ",node->myTag.index);
/*
...
//printf("%i and %i\n",splitNode1->myTag.index,splitNode2->myTag.index);
/*
 *          If the split nodes are connected, set the glide planes
 *          to those determined above.
 */
            if (armIndex1Max != -1) {            
                splitNode1->nx[armIndex1Max] = nplaneMax[X];
                splitNode1->ny[armIndex1Max] = nplaneMax[Y];
                splitNode1->nz[armIndex1Max] = nplaneMax[Z];

                splitNode2->nx[armIndex2Max] = nplaneMax[X];
                splitNode2->ny[armIndex2Max] = nplaneMax[Y];
                splitNode2->nz[armIndex2Max] = nplaneMax[Z];
            }
 ...
 */
		...
		(*splitNode1)->newNode = 1;
		(*splitNode2)->newNode = 1;
