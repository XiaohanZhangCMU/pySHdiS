...
/*
 *	When RijmTable is used, the number of cells must be 4 or more in each direction
 *	in order to satisfy the minimum image convention
 */
		if ( (param->nXcells < 4) || (param->nYcells < 4) || (param->nZcells < 4)  ) {
			Fatal("Number of cells in a dimension must be at least 4");
		}
#endif
        }
