 ...
 *          paradisgen [-cubel <cubelength>] [-help][-maxseg <maxseglen]    \
 *                     [-nloops <numloops>] [-seed <seedval]                \
 *                     [-radius <radius>] [-type <configtype>]
 *
 *      For <type> "fcc-line-loop"
 *
 *          paradisgen [-cubel <cubelength>] [-help][-maxseg <maxseglen]    \
 *                     [-nloops <numloops>] [-nchains <numchains>]          \
 *                     [-seed <seedval] [-radius <radius>]                  \
 *                     [-type <configtype>]
 *
 *      Overview:
 *
...
FuncData_t funcData[FTYPE_MAX] = {
        ...
        {FTYPE_FCC_LOOP,         FNAME_FCC_LOOP},
        {FTYPE_FCC_LINE_LOOP,    FNAME_FCC_LINE_LOOP},
...
static void Usage(char *program)
{
        ...
        printf("               [-seed <seedval] [-type <configtype>]\n");
        printf("\n");
        printf("    For <type> 'fcc-loop'\n");
        printf("\n");
        printf("    %10s [-cubel <cubelength>] [-help] \n", program);
        printf("               [-maxseg <maxseglen] [-radius <radius>] \n");
        printf("               [-nloops <numloops>] \n");
        printf("               [-seed <seedval] [-type <configtype>]\n");
        printf("\n");
        printf("    For <type> 'fcc-line-loop'\n");
        printf("\n");
        printf("    %10s [-cubel <cubelength>] [-help] \n", program);
        printf("               [-maxseg <maxseglen] [-radius <radius>] \n");
        printf("               [-nloops <numloops>] [-nchains <numchains>] \n");
        printf("               [-seed <seedval] [-type <configtype>]\n");
        printf("\n");
...
static void PrintArgs(InArgs_t *inArgs)
{
        ...
        switch (inArgs->type) {
            ...
            case FTYPE_FCC_LOOP:
                printf("Number of Loops:          %d\n" , inArgs->numLoops);
                printf("Radius of the loops:      %lf\n", inArgs->radius);
                printf("Length of each segment:   %lf\n", inArgs->maxSegLen);
                printf("Seed Value:               %d\n" , inArgs->seed);
                break;

            case FTYPE_FCC_LINE_LOOP:
                printf("Number of Chains:         %d\n" , inArgs->numChains);
                printf("Number of Loops:          %d\n" , inArgs->numLoops);
                printf("Radius of the loops:      %lf\n", inArgs->radius);
                printf("Length of each segment:   %lf\n", inArgs->maxSegLen);
 ...
 */
        ...
        case FTYPE_FCC_IRRAD:
                ...
                break;
		case FTYPE_FCC_LOOP:		
				CreateFCCglideLoop(&home, &inData, inArgs.cubeLength,
                                   inArgs.numLoops, inArgs.radius, inArgs.maxSegLen, 
								   inArgs.seed, &totDislocLen, inArgs.type);
				break;
		case FTYPE_FCC_LINE_LOOP:		
				CreateFCClineAndPrismLoop(&home, &inData, inArgs.cubeLength, inArgs.numChains,
                                          inArgs.numLoops, inArgs.radius, inArgs.maxSegLen, 
								          inArgs.seed, &totDislocLen, inArgs.type);
				break;
